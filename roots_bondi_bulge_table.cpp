...
{
	ofstream output;
	string filename = "bondi_roots_bulge.txt"; //edit this to change filename that the table will go to
	...
	lambda = (.25*exp(1.5))*12.123948; //this is the new critical value for lambda when bulge is added
...
DP fx(const DP v)
{
        return v*v/2.0+log(lambda)-log(position*position)-log(v)-1.0/position-(340.0/(position+135.754));
}
